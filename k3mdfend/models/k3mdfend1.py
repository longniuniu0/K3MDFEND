from Process.Graph import GlobalContrastiveLearning, SCL
from .layers import *
...
class KairosModel(nn.Module):
    ...
    def forward(self,data, flag):
        ...
        if flag == 'train':
            ...
            # 对抗目标：让融合评论逼近主内容
            adv_loss = (F.mse_loss(fused_comments, content_feat.detach()) * self.adv_weight)
            ...
            alp = 0.1
            main_loss = (1 - alp) * fake_CEloss + alp * (health_scloss + fake_scloss)
